...
//            print(account)
            // 归档
            NSKeyedArchiver.archiveRootObject(account, toFile: CZUserAccountViewModel.shareInstance.path)
            
            // 将account对象设置到单例对象中
            CZUserAccountViewModel.shareInstance.account = account
            
            // 退出当前控制器,并进去欢迎界面
            self.dismissViewControllerAnimated(false, completion: { () -> Void in
                UIApplication.sharedApplication().keyWindow?.rootViewController = CZWelcomeViewController()
            })
